---
title: "US Public Transit: Ridership Drivers and the COVID-19 Impact (2005-2022)"
...
background = "#fff1eaff" # don't change
```
...
```{r}
#| content: valuebox
#| title: "Peak Ridership (Pre-COVID)"
list(
  icon = "graph-up-arrow",
  color = "success",
...
```{r}
#| content: valuebox
#| title: "Lowest Point (COVID Trough)"
list(
...
public_transit_trend <- gocdata$ridership_year %>% 
  ...
  theme(
    ...
    panel.grid.major = element_line(color = "grey85", size = 0.6) # grid chính
    )+
...
#| fig-height: 4
#| fig-width: 12
#| title: "Bus Ridership & Infrastructure Investment correlation by Period"
...
*Bus ridership is counter-cyclical – it increases when GDP declines and unemployment rises.*
...
heatmap_bus <- ggcorrplot(cor_bus,lab = TRUE, type="lower", 
  ...
  #labs(subtitle = "Green = Negative, Red = Positive | Closer to ±1 = Stronger correlation")
heatmap_bus
```
...
:::{.card title= "Influential Factors on Ridership (Pre-COVID)"}

*The growth of real GDP exerts the strongest impact on bus ridership.*
...
#| fig-height: 4
#| fig-width: 12
#| title: "Rail Ridership & Infrastructure Investment correlation by Period"
...
##### Column - Heatmap - RAIL {width="50%"}

::: {.card title="Correlation heatmap"} 
*Infrastructure investment (spending_transit) and economic growth (real_gdp) show the strongest positive correlations with Urban Rail ridership*
...
*Economic growth (Real GDP) is by far the strongest positive driver of Urban Rail ridership.*
...
```{r}
#| label: Scatter plot Other modes
#| fig-width: 12
#| fig-height: 4
#| fig-dpi: 300
#| fig-asp: 0.33
#| fig-align: center
#| title: "Other modes Ridership & Infrastructure Investment correlation by Period"
...
*"Other modes" ridership is strongly and positively correlated with infrastructure investment (spending_transit) and economic growth (real_gdp)*

```{r}
#| label: Heatmap other mode
#| fig-dpi: 300
#| fig-asp: 1
#| fig-bg: "#fff3d4"
#| fig-height: 12
...
##### Column - coefficient plot - Other MODES {width = 50%}

:::{.card title = "Influential Factors on Ridership (Pre-COVID)"}

*"Other Modes" ridership is driven by a mix of factors. While economic growth and gas prices provide a strong positive push, rising diesel prices act as the most significant deterrent*
